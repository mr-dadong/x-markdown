import { app, BrowserWindow, Menu, dialog, ipcMain, MenuItemConstructorOptions, net, protocol, session, shell } from 'electron'
...
const updateManifestUrl = 'https://cnb.cool/X-2026/xmd-released/-/git/raw/main/version.json'
const installerExtensions = new Set(['.exe', '.msi', '.dmg', '.pkg', '.appimage', '.deb', '.rpm'])
...
function isUpdateManifest(value: unknown): value is UpdateManifest {
  ...
  return manifest.releases.every((item) => {
    ...
    return typeof release.title === 'string'
...
function selectUpdate(release: UpdateRelease): SelectedUpdate {
  ...
  if (!platform || !platform.available) throw new Error(`v${release.version} 暂未提供当前系统的安装包`)

  const selectedPackage = platform.packages.find((item) => item.arch === process.arch)
  if (!selectedPackage) throw new Error(`v${release.version} 暂未提供 ${process.arch} 架构的安装包`)
  if (!selectedPackage.filename || !selectedPackage.url) throw new Error('安装包信息不完整')
...
function ensureHttpsUrl(url: string): URL {
  ...
  if (parsedUrl.protocol !== 'https:') throw new Error('更新地址必须使用 HTTPS')
  return parsedUrl
}

// 只接收应用支持且真实存在的文件，避免把 Electron 自身的启动参数误当成文档。
async function getFilePathsFromArguments(commandLine: string[]): Promise<string[]> {
  ...
  const checkedPaths = await Promise.all(candidates.map(async (filePath) => {
    const isFile = await fs.promises.stat(filePath).then((stats) => stats.isFile()).catch(() => false)
    return isFile ? filePath : null
  }))
  return checkedPaths.filter((filePath): filePath is string => filePath !== null)
}
...
async function openPendingFiles(): Promise<void> {
  ...
  files.forEach((file) => mainWindow?.webContents.send(IPC_CHANNELS.menuOpenFile, file))
}
...
function createWindow(): void {
  ...
  mainWindow.webContents.setWindowOpenHandler(({ url }) => {
    try {
      const parsedUrl = new URL(url)
      if (['http:', 'https:', 'mailto:'].includes(parsedUrl.protocol)) void shell.openExternal(parsedUrl.toString())
    } catch {
      // 无效地址直接拒绝，不让它进入应用窗口。
    }
    return { action: 'deny' }
...
ipcMain.handle(IPC_CHANNELS.checkForUpdates, async () => {
  ...
  const latestRelease = manifest.releases.find((release) => release.version.trim() === manifest.latest.trim())
  if (!latestRelease) throw new Error(`找不到最新版 v${manifest.latest} 的发布信息`)
...
ipcMain.handle(IPC_CHANNELS.downloadUpdate, async (_event, updateUrl: string) => {
  const parsedUrl = ensureHttpsUrl(updateUrl)
  const response = await net.fetch(parsedUrl.toString(), { method: 'HEAD', redirect: 'follow' })
  ...
  return new Promise<{ status: 'downloaded'; filePath: string }>((resolve, reject) => {
    const onWillDownload = (_downloadEvent: Electron.Event, item: Electron.DownloadItem): void => {
      ...
      const savePath = path.join(app.getPath('temp'), `XMD-update-${Date.now()}${extension}`)
      item.setSavePath(savePath)
      item.on('updated', () => {
        const totalBytes = item.getTotalBytes()
        const receivedBytes = item.getReceivedBytes()
        const percent = totalBytes > 0 ? Math.min(100, Math.round(receivedBytes / totalBytes * 100)) : 0
        mainWindow?.webContents.send(IPC_CHANNELS.updateDownloadProgress, { percent, receivedBytes, totalBytes })
      })
...
ipcMain.handle(IPC_CHANNELS.installUpdate, async (_event, filePath: string) => {
  ...
  if (!resolvedPath.startsWith(`${tempDirectory}${path.sep}`) || !installerExtensions.has(path.extname(resolvedPath).toLowerCase())) {
    throw new Error('安装包路径无效')
  }
  const installerExists = await fs.promises.stat(resolvedPath).then((stats) => stats.isFile()).catch(() => false)
...
// IPC 通信处理
ipcMain.handle(IPC_CHANNELS.saveFile, async (
  _event,
  {
    filePath,
    content,
    ...
    if (filePath) {
      ...
      if (!force && expectedModifiedTime !== null) {
        ...
        if (stats.mtimeMs !== expectedModifiedTime) {
          return { success: false, conflict: true, error: '文件已被其他程序修改' }
        }
      }
      await writeTextFileAtomically(authorizedPath, content)
      const stats = await fs.promises.stat(authorizedPath)
      return { success: true, filePath: authorizedPath, modifiedTime: stats.mtimeMs }
    } else {
      ...
      if (!result.canceled && result.filePath) {
        ...
        return { success: true, filePath: result.filePath, modifiedTime: stats.mtimeMs }
      }
...
ipcMain.handle(IPC_CHANNELS.getDirectoryName, async (_event, filePath: string) => {
  return path.dirname(assertAuthorizedPath(filePath))
})

// 根据文档位置生成可迁移的相对地址；未保存文档使用标准本地文件地址。
function createEditorFileUrl(filePath: string, currentDocumentPath: string | null): string {
  if (!currentDocumentPath) return pathToFileURL(filePath).href

  const relativePath = path.relative(path.dirname(currentDocumentPath), filePath)
  return relativePath
    .split(path.sep)
...
async function getAvailableAttachmentPath(directoryPath: string, fileName: string): Promise<string> {
  const extension = path.extname(fileName)
  const baseName = path.basename(fileName, extension)
  let candidatePath = path.join(directoryPath, fileName)
  let index = 1
  while (await fs.promises.access(candidatePath).then(() => true).catch(() => false)) {
...
async function copyAttachmentWithProgress(
  ...
  const reportProgress = (status: AttachmentCopyProgress['status'], error?: string): void => {
...
function resolveEditorFilePath(url: string, currentDocumentPath: string | null): string {
  if (currentDocumentPath) assertAuthorizedPath(currentDocumentPath)
  const resolvedPath = url.startsWith('file:')
    ? fileURLToPath(url)
    : path.resolve(currentDocumentPath ? path.dirname(currentDocumentPath) : process.cwd(), decodeURIComponent(url))
...
async function createVideoResponse(request: Request, filePath: string): Promise<Response> {
  const stats = await fs.promises.stat(filePath)
  const fileSize = stats.size
  const contentType = videoMimeTypes[path.extname(filePath).toLowerCase()] ?? 'application/octet-stream'
  ...
  const requestedStart = suffixLength === null ? Number(match[1]) : Math.max(0, fileSize - suffixLength)
...
async function importEditorFile({
  ...
  const shouldCopyToAssets = kind === 'image' || (kind === 'file' && attachmentHandling === 'copy-to-assets')
  if (shouldCopyToAssets && !currentDocumentPath) throw new Error('复制资源前需要先保存当前文档')

  // 拖入和粘贴都使用同一套路径处理，确保生成的 Markdown 地址格式完全一致。
  let importedPath = sourcePath
  if (shouldCopyToAssets && currentDocumentPath) {
    const assetsDirectory = path.join(path.dirname(currentDocumentPath), 'assets')
    await fs.promises.mkdir(assetsDirectory, { recursive: true })
    const directTargetPath = path.join(assetsDirectory, path.basename(sourcePath))
    if (path.resolve(sourcePath) !== path.resolve(directTargetPath)) {
      importedPath = await getAvailableAttachmentPath(assetsDirectory, path.basename(sourcePath))
...
ipcMain.handle(IPC_CHANNELS.importEditorFile, async (_event, options) => importEditorFile(options))
...
ipcMain.handle(
  ...
  async (
    ...
    const filters = {
      image: [{ name: '图片', extensions: ['png', 'jpg', 'jpeg', 'gif', 'webp', 'svg', 'bmp'] }],
      video: [{ name: '视频', extensions: ['mp4', 'webm', 'ogg', 'mov', 'm4v'] }],
    ...
    if (shouldCopyToAssets && currentDocumentPath) {
      ...
      const assetsDirectory = path.join(path.dirname(authorizedDocumentPath), 'assets')
      await fs.promises.mkdir(assetsDirectory, { recursive: true })
      const directTargetPath = path.join(assetsDirectory, path.basename(selectedFilePath))
      if (path.resolve(selectedFilePath) !== path.resolve(directTargetPath)) {
        filePath = await getAvailableAttachmentPath(assetsDirectory, path.basename(selectedFilePath))
        if (kind === 'file' && requestId) {
          await copyAttachmentWithProgress(selectedFilePath, filePath, requestId)
        } else {
...
ipcMain.handle(
  ...
  async (_event, { url, currentDocumentPath }: { url: string; currentDocumentPath: string | null }) => {
...
ipcMain.handle(
  ...
  async (
    _event,
    { bytes, mimeType, currentDocumentPath }: { bytes: Uint8Array; mimeType: string; currentDocumentPath: string | null },
  ) => {
    if (!mainWindow) return null
    ...
    const extensionEntry = Object.entries(editorImageMimeTypes).find(([, value]) => value === mimeType)
    if (!extensionEntry) throw new Error('剪贴板中的图片格式不受支持')
    const authorizedDocumentPath = assertAuthorizedPath(currentDocumentPath)
    const assetsDirectory = path.join(path.dirname(authorizedDocumentPath), 'assets')
    await fs.promises.mkdir(assetsDirectory, { recursive: true })
    const targetPath = await getAvailableAttachmentPath(assetsDirectory, `粘贴图片${extensionEntry[0]}`)
...
ipcMain.handle(
  ...
  async (_event, { url, currentDocumentPath }: { url: string; currentDocumentPath: string | null }) => {
    ...
    return readEditorImageDataUrl(resolveEditorFilePath(url, currentDocumentPath))
  },
)
...
app.whenReady().then(async () => {
  protocol.handle('xmd-media', async (request) => {
    ...
    if (mediaUrl.host !== 'video' || !filePath) return new Response('视频地址无效', { status: 400 })
    const authorizedPath = assertAuthorizedPath(filePath)
