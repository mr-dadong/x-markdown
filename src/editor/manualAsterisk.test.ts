...
test("逐字输入 2*3*4 → Typography 抢先把 * 变乘号，不存在斜体歧义", () => {
  ...
  try {
    ...
    assert.equal(editor.getMarkdown(), "2×3×4");
  } finally {
    editor.destroy();
  }
});
...
test("非数字间输入 * → 保持字面星号 → 序列化保持原样（Typora 风格）", async () => {
  ...
  try {
    ...
    assert.equal(editor.getMarkdown(), "重点 * 请注意");
  } finally {
    editor.destroy();
  }
});
...
test("紧贴文字的 * 仍然保护性转义（往返不变）", async () => {
  ...
  try {
    ...
    editor.commands.setContent("价格 \\*低\\* 到难以置信", { contentType: "markdown" });
    assert.equal(
      editor.getMarkdown(),
      "价格 \\*低\\* 到难以置信",
...
test("字面星号存盘→重开往返自洽（Typora 风格）", async () => {
  ...
  try {
    await waitForCreate();
    second.commands.setContent(saved, { contentType: "markdown" });
    assert.equal(second.state.doc.textContent, "重点 * 请注意");
    assert.equal(second.getMarkdown(), saved);
  } finally {
    second.destroy();
  }
});
...
test("手动逐字输入 **2** → 输入规则立即变加粗 → 序列化保持 **2**", () => {
  ...
  try {
    ...
    assert.equal(editor.getMarkdown(), "**2**");
  } finally {
    editor.destroy();
  }
});
